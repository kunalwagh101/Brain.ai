...
| S-06.02.01 | E-06 | AI/API usage and budgets | backend/tests/test_ai_usage.py + backend/tests/test_ai_usage_reconciliation.py + backend/tests/test_ai_cached_cost.py + backend/tests/test_ai_cost_routes.py + UAT/F-06.02.md | backend/app/ai_usage.py + backend/app/ai_usage_models.py + backend/app/ai_usage_reconciliation.py + backend/app/routes/ai_usage.py + backend/migrations/versions/20260907_0012_ai_usage_cost_budgets.py + backend/migrations/versions/20260910_0016_ai_cached_input_cost.py | DONE |
| S-06.03.01 | E-06 | external API credential registry | backend/tests/test_api_registry.py + backend/tests/test_api_registry_worker.py + backend/tests/test_api_registry_usage_route.py + backend/tests/test_secrets.py | backend/app/api_registry.py + backend/app/routes/api_registry.py + backend/app/routes/api_registry_usage.py + backend/app/api_registry_worker.py | DONE |
...
| S-08.01.01 | E-08 | governed agent runtime | backend/tests/test_agent_policy.py + backend/tests/test_agent_runtime.py + backend/tests/test_agent_routes.py + backend/tests/test_agent_kill_switch.py + backend/tests/test_agent_maintenance.py + UAT/F-08.01.md | backend/app/agent_models.py + backend/app/agent_runtime.py + backend/app/agent_tools.py + backend/app/agent_maintenance.py + backend/app/agent_worker.py + backend/app/routes/agents.py + backend/migrations/versions/20260909_0015_agent_runtime.py + docs/AGENT_RUNTIME.md | DONE |
| S-09.01.01 | E-09 | production observability and SLOs | backend/tests/test_observability.py | backend/app/observability.py + backend/app/health.py + backend/app/integrations.py + backend/app/raw_events.py + backend/app/ai_provider_registry.py | DONE |
...
| S-10.07.01 | E-10 | developer/agent workspace | `backend/tests/test_agent_workspace.py` + `tests/agent-workspace-contract.test.mjs` + `UAT/F-10.07.md` | `backend/app/agent_workspace.py` + `backend/app/agent_workspace_models.py` + agent-workspace routes + `app/agent-workspace-api.ts` + `app/agent-workspace-bff.ts` + `app/agent-workspace-panel.tsx` + WorkOS agent route templates | BLOCKED |
...
| S-10.09.01 | E-10 | personal Activity & Notifications inbox | `backend/tests/test_activity.py` + `backend/tests/test_activity_inbox.py` + `tests/activity-contract.test.mjs` + `UAT/F-10.09.md` | `backend/app/activity.py` + `backend/app/activity_inbox.py` + activity models/routes + migrations `20260918_0025`/`0026`/`0027` + `app/activity-api.ts` + `app/activity-bff.ts` + `app/activity-panel.tsx` + `app/activity-dock.tsx` + WorkOS Activity route templates | BLOCKED |
...
S-02.04.01 is engineering-DONE on verified branch commit `8be45dd4d96a87d9fa5a3cf9a385f8cd3ca6349f`. Backend CI `35930622742` passed Ruff and all 385 backend tests; Delivery Verifier `35930622825` passed; Release Gate `35930622874` passed PostgreSQL upgrade/downgrade/forward recovery, backup/restore, frontend verification, image build and readiness smoke. Representative deployed/manual file and browser UAT remains UAT_PENDING.
...
S-05.02.01 is engineering-DONE on code commit `d6a7f974c4f02e0b6232e7ccbee5dcab98f0a689`. Permission-aware retrieval/no-evidence behavior, strict grounded JSON, bounded server-issued citations, cross-tenant/revocation protection, prompt-injection boundary, evaluation-gate enforcement, runtime discovery and cache-aware cost integration are covered by the passing automated suite. Real provider compatibility/cost smoke, representative >=90% retrieval evaluation, >=98% human semantic citation review, p95 <10s and authenticated WorkOS browser acceptance remain `UAT_PENDING` and are required before PASSED/production-accepted claims.
...
S-06.02.01 is engineering-DONE on code commit `d6a7f974c4f02e0b6232e7ccbee5dcab98f0a689`. Integer nano-USD cost resolution, cached-token accounting, explicit unknown-cost states, tenant-validated attribution, idempotent reconciliation, deduplicated threshold alerts, hard-budget blocking and spend read permissions are covered by the passing automated suite. Real provider/invoice reconciliation and authenticated admin workflow remain `UAT_PENDING`.
...
S-06.03.01 is engineering-DONE on verified branch commit `8be45dd4d96a87d9fa5a3cf9a385f8cd3ca6349f`. The suite covers tenant isolation, secret-reference-only persistence/API output, owner/scope history, rotation, fail-closed revocation, expiry cleanup and idempotent usage observation. Backend CI `35930622742`, Delivery Verifier `35930622825` and Release Gate `35930622874` passed. Real secret-manager/calling-system/frontend acceptance remains UAT_PENDING.

S-09.01.01 is engineering-DONE on verified branch commit `8be45dd4d96a87d9fa5a3cf9a385f8cd3ca6349f`. The suite covers secret redaction, allowlisted structured logging, bounded Prometheus dimensions, metrics auth, request IDs, production OTLP/metrics configuration and liveness/readiness separation. Backend CI `35930622742`, Delivery Verifier `35930622825` and Release Gate `35930622874` passed. Deployed telemetry, alert firing, representative SLO measurement and browser correlation remain UAT_PENDING.
...
S-08.01.01 is engineering-DONE on code commit `d6a7f974c4f02e0b6232e7ccbee5dcab98f0a689`. Missing policies default deny; tool risk classes constrain allowed policy modes; requester/cross-tenant isolation, objective binding, high-risk approval pause, rejection cleanup, agent disable kill switch and bounded audit evidence are covered by automated tests. The runtime uses F-06.01 for provider execution and F-09.02 for audit. Real PostgreSQL advisory-lock concurrency, live provider/budget/permission-loss behavior and frontend approval workflow remain `UAT_PENDING`.
...
EVIDENCE S-02.04.01
tests: backend/tests/test_evidence_ingestion.py
command: cd backend && pytest -q tests/test_evidence_ingestion.py tests/test_ask_brain_generic_evidence.py tests/test_decision_memory_generic_evidence.py
result: passed (closure verifier; baseline Backend CI 35930622742: 385 passed, 1 warning; Release Gate 35930622874: success)
code: backend/app/evidence_ingestion.py:172-839
commit: 8be45dd4d96a87d9fa5a3cf9a385f8cd3ca6349f

EVIDENCE S-06.03.01
tests: backend/tests/test_api_registry.py
command: cd backend && pytest -q tests/test_api_registry.py tests/test_api_registry_worker.py tests/test_api_registry_usage_route.py tests/test_secrets.py
result: passed (closure verifier; baseline Backend CI 35930622742: 385 passed, 1 warning; Release Gate 35930622874: success)
code: backend/app/api_registry.py:169-640
commit: 8be45dd4d96a87d9fa5a3cf9a385f8cd3ca6349f

EVIDENCE S-09.01.01
tests: backend/tests/test_observability.py
command: cd backend && pytest -q tests/test_observability.py
result: passed (closure verifier; baseline Backend CI 35930622742: 385 passed, 1 warning; Release Gate 35930622874: success)
code: backend/app/observability.py:143-353
commit: 8be45dd4d96a87d9fa5a3cf9a385f8cd3ca6349f

CHECK VERIFICATION S-10.07.01 / S-10.08.01 / S-10.09.01 — 2026-09-24

Repository head `1476b2d09625211b6ece309abe4fb0bade52a955` completed Backend CI `35982630115`, Delivery Verifier `35982630159` and Release Gate `35982629961` successfully. Backend CI passed Ruff and 385 backend tests. Release Gate passed 385 backend tests, frontend build/source contracts (138 passed, 0 failed, 1 skipped), repository verification, PostgreSQL upgrade/downgrade/forward recovery, backup/restore, production-image build and readiness smoke.

S-10.07.01: repository implementation and automated consumers are verified. It remains BLOCKED because its accepted UAT contract requires official WorkOS activation and authenticated browser testing of project/channel revocation, requester isolation, approval-gated action, credential non-exposure and accessibility. The current package/lockfile does not include the WorkOS AuthKit packages, so that evidence cannot be produced honestly from this branch.

S-10.08.01: repository implementation and automated consumers are verified. It remains BLOCKED because its accepted UAT contract additionally requires authenticated Owner/Admin/Member browser testing and real configured secret-store create/rotate/revoke tests with disposable credentials.

S-10.09.01: repository implementation, frontend contracts and Activity migration chain `20260918_0025 -> 0026 -> 0027` are verified, including downgrade and forward recovery. It remains BLOCKED by S-10.07/S-10.08 acceptance, blocked S-07.01, and official WorkOS authenticated multi-user browser UAT. No production PASSED/DONE claim is made.

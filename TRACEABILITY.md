...
| S-05.02.01 | E-05 | evidence-backed Ask Brain | backend/tests/test_ask_brain.py + backend/tests/test_ask_brain_routes.py + backend/tests/test_ask_brain_citations.py + backend/tests/test_ask_brain_limits.py + backend/tests/test_ask_brain_evaluation.py + backend/tests/test_search_evaluation.py + UAT/F-05.02.md | backend/app/ask_brain.py + backend/app/routes/ask_brain.py + backend/app/ask_brain_evaluation.py + scripts/run-ask-brain-evaluation.py + scripts/score-ask-brain-review.py + ops/evals/ask-brain.dataset.example.json | BLOCKED |
...
| S-09.04.01 | E-09 | latency/cost benchmarks | backend/tests/test_performance_budget.py + UAT/F-09.04.md | backend/app/performance_budget.py + scripts/run-performance-benchmark.py + ops/performance/budgets.json + .github/workflows/performance-gate.yml + docs/PERFORMANCE.md | BLOCKED |
...
S-05.01.01 intentionally has no EVIDENCE block yet. GitHub Actions has repeatedly failed before runner startup (`runner_id=0`/no job steps), so there is no truthful passing test result to record and the story remains IN_REVIEW.

S-05.02.01 intentionally has no EVIDENCE block yet. The production slice is staged on `increment-10-ai-provider-gateway` through implementation commit `f4ff8217ac02b6392cede6e709ccdbcab124b33f`: permission-aware/no-evidence retrieval, governed explicit provider/model execution, fail-closed structured grounding, bounded citation excerpts, an Ask-Brain-specific 8,192-token output ceiling, validation/authz/revocation/tenant tests, and a two-phase deployed evaluation that separates automatic retrieval/security metrics from human-reviewed semantic claim-citation correctness. Sensitive `.local/` and `.evaluation/` UAT artifacts are gitignored and regression-tested. Backend CI run 34405201146 for that implementation failed before any workflow step; no Ruff/Pytest evidence exists. S-05.01.01 remains unverified, OQ-005 remains unresolved for production provider/model/data policy, and the real >=90% retrieval, >=98% human-reviewed citation correctness, latency/cost and authenticated frontend/manual UAT have not run. The story therefore remains BLOCKED and no passing/correctness/performance claim is made.
...
S-09.03.01 intentionally has no EVIDENCE block yet. A provider-neutral Release Gate, full Alembic upgrade/downgrade/forward-recovery exercise, production-mode container readiness smoke, guarded PostgreSQL backup/restore scripts, restored-data sentinel verification, release manifest, regression tests, runbook and UAT are staged. The gate has not executed on a runner; GitHub required-check enforcement is not available/verified on the current private-repository setup; and OQ-007 still blocks the environment-specific registry/runtime deployment and rollback drill. The story therefore remains BLOCKED and no recovery-exercise success is claimed.

S-09.04.01 intentionally has no EVIDENCE block yet. Deterministic percentile/budget evaluation, a versioned core/search/AI workload, async deployed HTTP runner, exact F-06.02 AI cost delta, fail-closed unknown-cost handling, a manually dispatched staging Performance Gate, regression tests, methodology and production-like UAT are staged. The underlying search/AI slices do not yet have executable passing verification, GitHub-hosted jobs still cannot obtain a runner, F-09.03 has not produced the target staging environment, and no benchmark report has actually run. The story therefore remains BLOCKED and no latency/throughput/cost performance claim is made.